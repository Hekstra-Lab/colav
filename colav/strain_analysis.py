...
def calculate_strain(
    ref_coords, def_coords, min_dist=6, max_dist=8, err_threshold=10, verbose=False
):
    ...
    for atom_idx in range(n_atoms):
        ...
        while err_num != it_num:
            ...
            in_range = np.where(
                (ref_dist[:, atom_idx] >= min_dist)
                & (ref_dist[:, atom_idx] <= max_dist + err_num)
            )
            ...
            for neighbor_idx in np.where(wn != 0)[0]:
                ...
                ref_d = (
                    (ref_coords[neighbor_idx, :] - ref_coords[atom_idx, :])
                    .reshape((3, 1))
                    .astype("float64")
                )
                def_d = (
                    (def_coords[neighbor_idx, :] - def_coords[atom_idx, :])
                    .reshape((3, 1))
                    .astype("float64")
                )
            ...
            Em = 0.5 * (
                np.eye(3) - np.linalg.inv(np.dot(Fm, Fm.T))
            )  # using Euler-Almansi
            ...
            if verbose:
                print(
                    f"pos_strain_tensor: {Em}\tpos_shear_tensor: {gamma}\tpos_shear_energy: {np.sum(np.square(gamma))}"
                )
...
def determine_shared_atoms(
    ...
    for i, struc in enumerate(structure_list):
        ...
        current_atom_set = set(
            [tuple(x) for x in df[["residue_number", "atom_name"]].values.tolist()]
        )

        # check if the sequence is completely intact and consecutive
        if (
            sorted(current_atom_set)[0][0]
            + len(np.unique(np.array(sorted(current_atom_set))[:, 0]))
            - 1
            == sorted(current_atom_set)[-1][0]
        ):
    ...
    ref_atom_set = atom_set.intersection(
        set(
            [
                tuple(x)
                for x in reference_ppdb[["residue_number", "atom_name"]].values.tolist()
...
def calculate_strain_dict(
    ...
    for i, struc in enumerate(sorted(filtered_strucs)):
        ...
        sheare, sheart, straint = calculate_strain(
            ref_strain_coords, current_strain_coords
        )
