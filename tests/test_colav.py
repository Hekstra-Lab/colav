...
def test_determine_shared_atoms(): 
    ...
    assert determine_shared_atoms(['tests/TEST1.pdb', 'tests/TEST2.pdb'], test_ppdbdf, (1,5), atoms=["N", "CA", "C"], alt_locs=[""]) \
    == (set([(1,"N"), (1,"CA"), (1,"C"), (2,"N"), (2,"CA"), (2,"C"), (3,"N"), (3,"CA"), (3,"C"), (4,"N"), (4,"CA"), (4,"C"), (5,"N"), (5,"CA"), (5,"C")]), ['tests/TEST1.pdb', 'tests/TEST2.pdb'])
...
def test_data_from_atoms(): 
    ...
    # testing using TEST1.pdb; see header of TEST1.pdb for more information 
    test_ppdbdf = PandasPdb().read_pdb('tests/TEST1.pdb').df['ATOM']
    assert_array_equal(coords_from_atoms(test_ppdbdf, [(1,"N"), (1,"CA"), (1,"C"), (2,"N"), (2,"CA"), (2,"C"), (3,"N"), (3,"CA"), (3,"C"), (4,"N"), (4,"CA"), (4,"C"), (5,"N"), (5,"CA"), (5,"C")]), np.array([
                [0.000,0.000,0.000], 
                [1.459,0.000,0.000], 
                [2.006,0.000,1.424],
                [1.468,0.869,2.285], 
    ...
    assert_array_equal(bfacs_from_atoms(test_ppdbdf, [(1,"N"), (1,"CA"), (1,"C"), (2,"N"), (2,"CA"), (2,"C"), (3,"N"), (3,"CA"), (3,"C"), (4,"N"), (4,"CA"), (4,"C"), (5,"N"), (5,"CA"), (5,"C")]),np.array(15*[40.00]))

def test_calculate_strain(): 

    # testing using TEST1.pdb and TEST2.pdb; see header of TEST1.pdb for more information 
    ref_coords = PandasPdb().read_pdb('tests/TEST3.pdb').df['ATOM'][['x_coord', 'y_coord', 'z_coord']].to_numpy()
    def_coords = PandasPdb().read_pdb('tests/TEST4.pdb').df['ATOM'][['x_coord', 'y_coord', 'z_coord']].to_numpy()
    shear_energy, shear_tensor, strain_tensor = calculate_strain(ref_coords, def_coords)
    assert_allclose(shear_energy, np.array([1.34821937, 1.34821937, 1.34821937, 1.34821937]))
    assert_allclose(shear_tensor, np.array([
    ...
    assert_allclose(strain_tensor, np.array([
               ...
               [[ 0.00000000e+00, -6.37512148e-01,  1.03164408e-17],
                [-6.37512148e-01, -7.89391419e-01,  2.44897959e-01],
                [ 3.54887521e-17,  2.44897959e-01,  0.00000000e+00]],
...
def test_calculate_strain_dict(): 
    ...
    strain_dict, shared_atom_set = calculate_strain_dict(['tests/TEST3.pdb', 'tests/TEST4.pdb'], 'tests/TEST3.pdb', (1,2), atoms=["N", "CA", "C"], alt_locs=[""], save=False)
    assert shared_atom_set == set([(1,"N"), (1,"CA"), (1,"C"), (2,"N")])
    assert_allclose(strain_dict['tests/TEST4.pdb']['sheare'], np.array([1.34821937, 1.34821937, 1.34821937, 1.34821937]))
    assert_allclose(strain_dict['tests/TEST4.pdb']['sheart'], np.array([
               [[ 2.63130473e-01, -6.37512148e-01, -1.10977327e-15],
                [-6.37512148e-01, -5.26260946e-01,  2.44897959e-01],
                [-1.15757414e-15,  2.44897959e-01,  2.63130473e-01]],
    ...
    assert_allclose(strain_dict['tests/TEST4.pdb']['straint'], np.array([
               [[ 3.88578059e-16, -6.37512148e-01, -1.10977327e-15],
                [-6.37512148e-01, -7.89391419e-01,  2.44897959e-01],
                [-1.15757414e-15,  2.44897959e-01,  1.11022302e-16]],
    ...
    assert strain_dict['tests/TEST4.pdb']['atom_list'] == sorted([(1,"N"), (1,"CA"), (1,"C"), (2,"N")])
...
def test_calculate_tl(): 
    ...
    assert_array_equal(calculate_dh_tl([1.,0.5,-1.,-0.5,-1.2,-0.8]), [1.5,1.7,1.8])
    assert_array_equal(calculate_pw_tl([1.,0.5,-1.,-0.5,-1.2,-0.8], (1,4)), [2.5,2.7,1.8,3])
    assert_array_equal(calculate_sa_tl([1.,0.5,-1.,-0.5,-1.2,-0.8,1.,0.5,-1.,-0.5,-1.2,-0.8], [(1,"N"),(1,"CA"),(1,"C"),(2,"N")]), [7.5,2.5])
